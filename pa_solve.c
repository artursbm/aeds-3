  ...
  for (int k=0; k<num_games; k++)
    {
      ...
      scanf("%s %s %d %d",a,b,&score_i,&score_j);
      i = get_team_number(a);
      j = get_team_number(b);
      assert(game[i][j]==0);
      game[i][j] = 1;
...
void visit (int i, int j)
{
  ...
  if (i>=num_teams)
    ...
    if(best_place[k] < num_better+1) {
      best_place[k] = best_place[k];
    }
    else   best_place[k] = num_better+1;
    
    if(worst_place[k] < num_teams - num_worse) {
      worst_place[k] = worst_place[k];
    }
    else   worst_place[k] = (num_teams-num_worse);
	}
      return;
    }
  if (i==j || game[i][j])
